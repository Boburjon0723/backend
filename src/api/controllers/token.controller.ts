...
export const getWalletConfig = async (req: Request, res: Response) => {
    try {
        const [settingsRes, treasuryRes] = await Promise.all([
            pool.query('SELECT admin_card_number FROM platform_settings ORDER BY updated_at DESC LIMIT 1'),
            pool.query('SELECT balance FROM platform_balance ORDER BY id ASC LIMIT 1')
        ]);

        const adminCardNumber = settingsRes.rows[0]?.admin_card_number || null;
        const systemAvailableMali = Number.parseFloat(treasuryRes.rows[0]?.balance || 0);

        res.status(200).json({
            admin_card_number: adminCardNumber,
            system_available_mali: Number.isFinite(systemAvailableMali) ? systemAvailableMali : 0
        });
    } catch (error) {
