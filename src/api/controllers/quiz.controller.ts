...
export const deleteQuiz = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        // @ts-ignore
        const mentorId = req.user.id;
        const ok = await QuizModel.deleteByIdForMentor(quizId, mentorId);
        if (!ok) {
            res.status(404).json({ message: 'Viktorina topilmadi yoki ruxsat yo‘q' });
            return;
        }
        res.status(200).json({ success: true });
    } catch (error) {
        console.error('Error deleting quiz:', error);
        res.status(500).json({ message: 'Viktorinani o‘chirib bo‘lmadi' });
    }
};
