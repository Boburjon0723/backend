...
export class QuizModel {
    ...
    /** Mentor o‘z viktorinasini o‘chiradi (savollar, variantlar, natijalar) */
    static async deleteByIdForMentor(quizId: string, mentorId: string): Promise<boolean> {
        const client = await pool.connect();
        try {
            await client.query('BEGIN');
            const own = await client.query(
                'SELECT id FROM quizzes WHERE id = $1 AND mentor_id = $2',
                [quizId, mentorId]
            );
            ...
            await client.query('DELETE FROM quiz_results WHERE quiz_id = $1', [quizId]).catch(() => {
                /* jadval bo‘lmasa */
            });
            await client.query(
