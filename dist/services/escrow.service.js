...
class EscrowService {
     ...
     */
    static async holdFunds(userId, serviceId, amount) {
        const client = await database_1.pool.connect();
        try {
            await client.query('BEGIN');
            // 1. Check User Balance & Lock Row
            const userBalanceRes = await client.query('SELECT balance FROM token_balances WHERE user_id = $1 FOR UPDATE', [userId]);
            ...
            await client.query(`UPDATE token_balances 
         SET balance = balance - $1, 
             locked_balance = locked_balance + $1 
         WHERE user_id = $2`, [amount, userId]);
            // 3. Create Escrow Record
            const escrow = await Escrow_1.EscrowModel.create(client, {
                user_id: userId,
                service_id: serviceId,
                amount: amount
            });
            // 4. Create Transaction Record (Escrow Hold)
            await Transaction_1.TransactionModel.create(client, {
                sender_id: userId,
    ...
    static async releaseFunds(escrowId) {
        ...
        try {
            ...
            if (escrow.status !== 'held')
                throw new Error(`Escrow status is ${escrow.status}, cannot release`);
           ...
           WHERE user_id = $2`, [amount, escrow.user_id]);
            // 4. Credit Provider
            await client.query(`UPDATE token_balances 
           SET balance = balance + $1, 
               lifetime_earned = lifetime_earned + $1 
           WHERE user_id = $2`, [netAmount, service.provider_id]);
            // 5. Credit Platform
            await client.query(`UPDATE platform_balance 
           SET balance = balance + $1, 
               total_commissions_collected = total_commissions_collected + $1 
           WHERE id = 1`, [commission]);
            // 6. Update Escrow Status
            const updatedEscrow = await Escrow_1.EscrowModel.updateStatus(client, escrowId, 'released');
     ...
     */
    static async refundFunds(escrowId) {
        const client = await database_1.pool.connect();
        try {
            await client.query('BEGIN');
            // 1. Get Escrow Record & Lock
            ...
            if (!escrow)
                throw new Error('Escrow record not found');
            if (escrow.status !== 'held')
                throw new Error(`Escrow status is ${escrow.status}, cannot refund`);
            const amount = parseFloat(escrow.amount);
            ...
            const updatedEscrow = await Escrow_1.EscrowModel.updateStatus(client, escrowId, 'refunded');
...
EscrowService.COMMISSION_RATE = parseFloat(process.env.SERVICE_COMMISSION_PERCENTAGE || '0.05'); // 5%
